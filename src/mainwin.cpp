...
void Mainwin::on_insert_customer_click() {
   Gtk::Dialog dialog{"Insert Customer", *this};
    ...
    grid.attach(t_email, 0, 1, 1, 1); 
    grid.attach(e_email, 1, 1, 2, 1); 
	
    Gtk::Label t_phone{"Phone"};       
    Gtk::Entry e_phone; 

    grid.attach(t_phone, 0, 2, 1, 1); 
    ...
    if (e_name.get_text().size() == 0) {e_name.set_text("Name required"); continue;}
	if (e_email.get_text().size() == 0) {e_email.set_text("Email required"); continue;}
	if (e_phone.get_text().size() == 0) {e_phone.set_text("Name required"); continue;}
